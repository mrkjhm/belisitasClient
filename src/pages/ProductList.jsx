        ...
        <div className="mx-auto container flex sm:flex-row flex-col gap-10 ">
          ...
          <div className="md:w-1/5 sm:w-1/3 sm:block hidden text-sm">
            ...
            <div className="flex sm:flex-col flex-wrap gap-3">
              ...
              {categories
                ?.filter((category) => product.some((item) => item.category?._id === category._id)) // Only show categories with products
                .map((category) => (
                  <button
                    key={category._id}
            ...
            <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-6 w-full">
              {currentProducts.length > 0 ? (
                currentProducts.map((item, index) => (
                  <div key={index} className="bg-white p-3 flex flex-col w-full hover:-translate-y-2 transition-all duration-500 ease-in-out hover:shadow-lg"> {/* Added w-full */}
